      ...
      <main>
        <div class="viewport margin-top">
          <div class="grid flex-between margin-bottom">
            ...
            <div>
              <button onClick={() => navigate('/admin/items')}>Back</button>
            </div>
          </div>
          {globalMessage && (
            <GlobalMessage
              ...
              <div class="grid">
                <label>
                  <input
                    type="radio"
                    name="status"
                    value={ItemStatus.InOffer}
                    checked={form.status === ItemStatus.InOffer}
                    onChange={() => setForm({ ...form, status: ItemStatus.InOffer })}
                  />{' '}
                  In offer
                </label>
                <label>
                  <input
                    type="radio"
                    name="status"
                    value={ItemStatus.NotListed}
                    checked={form.status === ItemStatus.NotListed}
                    onChange={() => setForm({ ...form, status: ItemStatus.NotListed })}
                  />{' '}
                  Not listed
                </label>
                <label>
                  <input
                    type="radio"
                    name="status"
                    value={ItemStatus.OfferAgreed}
                    checked={form.status === ItemStatus.OfferAgreed}
                    onChange={() => setForm({ ...form, status: ItemStatus.OfferAgreed })}
                  />{' '}
                  Offer agreed
                </label>
              </div>
              ...
              <div class="grid">
                <label>
                  <input
                    type="radio"
                    name="tradeType"
                    value={TradeType.post}
                    checked={form.tradeType === TradeType.post}
                    onChange={() => setForm({ ...form, tradeType: TradeType.post })}
                  />{' '}
                  Post
                </label>
                <label>
                  <input
                    type="radio"
                    name="tradeType"
                    value={TradeType.delivery}
                    checked={form.tradeType === TradeType.delivery}
                    onChange={() => setForm({ ...form, tradeType: TradeType.delivery })}
                  />{' '}
                  Delivery
                </label>
                <label>
                  <input
                    ...
                    checked={form.tradeType === TradeType.pickup}
                    onChange={() => setForm({ ...form, tradeType: TradeType.pickup })}
                  />{' '}
                  Pickup
                </label>
              </div>
